...
//   const [isDarkMode, setIsDarkMode] = useState(false); // State untuk Theme
...
//   return (
//     <div className={`fixed inset-0 w-screen h-screen overflow-hidden font-sans p-0 select-none flex flex-col box-border transition-colors duration-500 ${themeClass}`}>
...
//       <div className="grid grid-cols-12 gap-0 h-full min-h-0 relative z-10">
        
//         {/* LEFT COLUMN */}
//         <div className="col-span-3 flex flex-col gap-0 min-h-0">
//           <div className="bg-blue-700 rounded-none p-8 border-r border-white text-white shadow-2xl relative overflow-hidden shrink-0 h-[28vh]">
...
//                 <h2 className="text-6xl -ml-1 font-black text-white tracking-tighter tabular-nums leading-none">
//                   {currentTime.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
//                 </h2>
//                 <Clock size={44} className="text-yellow-300 drop-shadow-lg" />
//               </div>
//               <p className="text-xs font-bold mt-4 flex items-center gap-3 opacity-90 uppercase tracking-widest">
//                 <Calendar size={14} />
...
//              <CompactStat theme={isDarkMode} label="Kunjungan" value={stats.totalKunjungan} icon={<Users size={24}/>} bgColor="bg-emerald-200 text-emerald-700" />
//              <CompactStat theme={isDarkMode} label="Siswa Aktif" value={stats.totalSiswa} icon={<LogIn size={24}/>} bgColor="bg-blue-200 text-blue-700" />
//              <CompactStat theme={isDarkMode} label="Di Dalam" value={stats.masihDiDalam} icon={<Activity size={24}/>} bgColor="bg-rose-200 text-rose-700" />
//           </div>
//         </div>

//         {/* MIDDLE COLUMN */}
//         <div className="col-span-6 flex flex-col gap-0 min-h-0">
//           <div className="flex-[2.5] bg-white rounded-none overflow-hidden relative min-h-0 group">
...
//                       className={`px-4 flex items-center justify-center text-center py-2 h-[57px] w-full font-black text-xs transition-all bg-blue-700 text-white`}
//                     >
//                       <p className={`${currentVideoIndex === i ? 'bg-white flex items-center justify-center text-blue-700 px-2 py-1 text-center w-[98%] h-[98%]' : ''} relative top-[-1px] flex items-center gap-2`}>
//                         <Youtube size={20} className='relative top-[0.9px]' />
//                         VIDEO {i + 1}
//                       </p>
//                     </button>
//                   ))}
...
//             {/* FLOATING CONTROLS (Theme & Video) */}
//             <div className="absolute bottom-[2px] left-1/2 -translate-x-1/2 z-[999] flex gap-3">
//               <button 
//                 onClick={() => setIsDarkMode(!isDarkMode)}
//                 className="p-2 rounded-full bg-blue-700 active:scale-[0.92] border-white flex items-center justify-center h-max text-white border-4 hover:bg-blue-800 transition-all"
//               >
//                 <Palette size={18} />
//               </button>
//             </div>

//             <div className={`w-full absolute bottom-0 ${isDarkMode ? 'bg-slate-900' : 'bg-white'} h-[20px] flex items-center justify-center left-0 z-[88]`}>
//             </div>
...
//           {/* <div className={`w-full h-[1px] ${isDarkMode ? 'bg-slate-800' : 'bg-blue-500'}`}></div> */}
//           <div className="relative flex-1 grid grid-cols-3 gap-0 min-h-0">
//             <CollectionCard theme={isDarkMode} label="Katalog" value={collection.totalBiblio} icon={<Library size={28}/>} />
//             <CollectionCard theme={isDarkMode} label="Fisik Buku" value={collection.totalEksemplar} icon={<BookCopy size={28}/>} />
//             <CollectionCard theme={isDarkMode} label="Serial" value={collection.totalSerial} icon={<Newspaper size={28}/>} />
//           </div>
//         </div>

//         {/* RIGHT COLUMN */}
//         <div className="col-span-3 flex flex-col gap-0 min-h-0">
//           <div className="bg-blue-700 rounded-none p-8 border-l border-white text-white shadow-2xl relative overflow-hidden shrink-0 h-[28vh]">
//             <div className="relative z-10">
//               <p className="text-[10px] font-bold uppercase tracking-widest text-blue-100 mb-1">Jakarta, ID</p>
//               <div className="flex items-center justify-between">
//                 <h4 className="text-6xl font-black text-white tracking-tighter tabular-nums leading-none">31°C</h4>
//                 <CloudSun size={50} className="text-yellow-300 drop-shadow-lg" />
//               </div>
//               <p className="text-xs font-bold mt-4 opacity-90 uppercase tracking-widest">Cerah Berawan</p>
//             </div>
//           </div>
...
//                 ].map((item) => (
//                   <div key={item.display} className={`flex h-full items-center justify-between py-4 px-8 border-b shadow-sm transition-all ${isDarkMode ? 'bg-white/5 border-slate-800' : 'bg-white/50 border-slate-200'}`}>
//                     <span className={`text-[16px] font-black uppercase tracking-wider ${isDarkMode ? 'text-blue-400' : 'text-black'}`}>{item.display}</span>
//                     <span className={`text-xl font-black tabular-nums ${isDarkMode ? 'text-white' : 'text-blue-700'}`}>{sholatTimes[item.key]}</span>
//                   </div>
//                 ))
//               ) : (
//                 <div className="flex-1 flex items-center justify-center text-xs">Memuat Jadwal...</div>
//               )}
//             </div>
//           </div>
//         </div>
...
//   <div className={`flex items-center justify-between px-8 py-4 border-b h-full shadow-sm transition-all ${theme ? 'bg-transparent border-slate-800' : 'bg-white border-slate-300'}`}>
//     <div className="flex items-center gap-4">
//       <div className={`p-3 rounded-2xl ${bgColor} shadow-sm`}>
//         {icon}
//       </div>
//       <span className={`text-[16px] font-black uppercase tracking-widest ${theme ? 'text-slate-400' : 'text-black'}`}>{label}</span>
//     </div>
//     <span className={`text-4xl font-black tabular-nums tracking-tight ${theme ? 'text-white' : 'text-slate-900'}`}>{value}</span>
//   </div>
// );

// const CollectionCard = ({ label, value, icon, theme }: any) => (
//   <div className={`flex flex-col items-center justify-center py-4 px-4 text-center relative transition-all ${theme ? 'bg-slate-900' : 'bg-white'}`}>
//     <div className="text-blue-500 mb-2">
//       {icon}
//     </div>
//     <span className={`text-3xl font-black tabular-nums ${theme ? 'text-white' : 'text-slate-900'}`}>{value.toLocaleString()}</span>
//     <span className={`text-[9px] font-extrabold uppercase tracking-[0.15em] mt-1 ${theme ? 'text-slate-500' : 'text-black'}`}>{label}</span>
//   </div>
// );
...
//   // Fungsi untuk memaksa video tetap berputar jika terdeteksi 'Pause'
//   const onStateChange = (event: any) => {
//     // 2 adalah code status YouTube untuk 'PAUSED'
//     if (event.data === 2) {
...
//   return (
//     <div className="absolute inset-0 w-full top-5 h-full bg-white overflow-hidden">
//       <YouTube 
...
//             rel: 0, 
//             iv_load_policy: 3,
//             disablekb: 1, // Mematikan spasi/tombol keyboard untuk pause
//           },
//         }} 
//         onEnd={onEnd}
//         onError={onEnd}
//         onStateChange={onStateChange} // Monitor perubahan status video
//         /* Scale 120% untuk memotong semua UI YouTube (Logo, Judul, Share, Tonton Nanti) */
//         className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[100.6%] h-[100.6%]"
//       />
      
//       {/* Overlay transparan tambahan untuk menangkap klik agar tidak tembus ke video */}
        ...
        <div className="md:col-span-6 flex flex-col gap-0 min-h-0 order-3 md:order-none">
          <div className="aspect-video md:flex-[2.5] bg-white rounded-none overflow-hidden relative min-h-0 group">
            ...
            <div className="absolute bottom-[-2px] left-1/2 -translate-x-1/2 z-[999] flex gap-3">
              <button 
                onClick={() => setIsDarkMode(!isDarkMode)}
                className="p-3 rounded-full bg-blue-700 active:scale-[0.92] border-white flex items-center justify-center h-max text-white border-2 hover:bg-blue-800 transition-all shadow-xl"
              >
                <Palette size={20} />
              </button>
            </div>
        ...
        <div className="md:col-span-3 flex flex-col gap-0 min-h-0 order-2 md:order-none">
          <div className="bg-blue-700 p-6 md:p-8 border-b md:border-l md:border-b-0 border-white text-white shadow-2xl relative overflow-hidden shrink-0">
            <div className="relative z-10">
              <p className="text-[10px] font-bold uppercase tracking-widest text-blue-100 mb-1">Jakarta, ID</p>
              <div className="flex items-center justify-between">
                <h4 className="text-4xl md:text-6xl font-black text-white tracking-tighter tabular-nums leading-none">31°C</h4>
                <CloudSun size={40} className="md:w-12 md:h-12 text-yellow-300 drop-shadow-lg" />
              </div>
              <p className="text-[10px] md:text-xs font-bold mt-3 md:mt-4 opacity-90 uppercase tracking-widest">Cerah Berawan</p>
            </div>
          </div>
